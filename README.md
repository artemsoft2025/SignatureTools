...
Таким образом пополняем базу сигнатур `base1.sigs` образцами других зловредов, важно использовать параметр --append для дописывания сигнатур, а не перезаписи файла с сигнатурами. Конечно нужно организовать резервные копии баз.
 <br>
Чтобы сканер использовал файл с сигнатурами используйте параметр `-f`:
```cmd
sighunter.exe C:\Windows\System32\ -f base1.sigs
```
Так же файлы с сигнатурами можно указывать в файле-конфигурации, об этом чуть ниже.
 <br>
...
sighunter.exe c:\
```
будут просканированы файлы с расширениями ".exe", ".dll", ".sys", ".com", ".scr", ".ocx", ".cpl", ".drv", ".efi" размером не более 500 мб на всем диске C:\ , база сигнатур берется из файла `custom_signatures.sigs`.
...
Набор утилит SignatureTools не ограничивается только поиском вредоносных файлов. Он предоставляет универсальный инструментарий для работы с бинарными сигнатурами различных типов файлов.

### Поиск любых файлов по сигнатурам

Примеры применения:
- Поиск специфических документов или медиа-файлов
- Обнаружение конфигурационных файлов с определенным содержимым
- Поиск архивов и резервных копий
- Идентификация файлов по уникальным бинарным паттернам

### Поиск похожих документов
Эффективное решение для контроля установленного ПО на рабочих местах:

Сценарии использования:
- Обнаружение нежелательных приложений на АРМ
- Мониторинг соблюдения политик лицензирования
- Поиск запрещенного программного обеспечения
